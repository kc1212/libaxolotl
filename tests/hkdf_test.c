...
static char* test_vector_v3()
{
	...
	int ret = hkdf_derive_secrets(out, HKDF_MESSAGE_V3, ikm, sizeof ikm,
			salt, sizeof salt, info, sizeof info, outlen);
	mu_assert("wrong ret", 0 == ret);
	mu_assert("wrong out", 0 == sodium_memcmp(out, okm, outlen));
	return 0;
}

static char* test_vector_long_v3()
{
	const unsigned char ikm[]  = {
...
static char* test_vector_v2()
{
	...
	int ret = hkdf_derive_secrets(out, HKDF_MESSAGE_V2, ikm, sizeof ikm,
			salt, sizeof salt, info, sizeof info, outlen);
	mu_assert("", 0 == ret);
	mu_assert("", 0 == sodium_memcmp(out, okm, outlen));
	return 0;
